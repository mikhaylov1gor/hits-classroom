...
function VotingPanel({
  ...
  // Candidates: submissions by team members
  const memberIds = new Set(team.members.map((m) => m.user_id))
  const candidates = submissions.filter((s) => memberIds.has(s.user_id))
  const myCandidate = candidates.find((s) => s.user_id === myUserId)
  ...
  const handleVote = useCallback(
    ...
    [courseId, assignmentId, team.id, onRefresh],
  )
        ...
        {candidates.map((sub) => {
          ...
          return (
            ...
            >
              <ListItemText
                primary={
                  <Typography variant="body2" fontWeight={isOwn ? 600 : 400}>
                    {authorName}
                    {isOwn && (
                      <Chip
                        label="Моё"
                ...
                secondary={sub.body ? sub.body.slice(0, 80) : 'Без описания'}
              />
              <ListItemSecondaryAction>
                <Button
                  size="small"
